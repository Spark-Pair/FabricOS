...
const Purchases: React.FC = () => {
  ...
  const handlePrint = () => {
    ...
    document.body.style.height = '218.5mm';

    window.print();
    document.body.innerHTML = originalContent;

    window.location.reload(); // optional but safer for React
  };
    ...
    <>
      ...
      >
        <div className="p-6 space-y-6">
                  ...
                  {invoiceItems.map((item, idx) => (
                    <div key={idx} className="p-3.5 flex items-center justify-between group">
                      ...
                      <div className="flex items-center gap-6 px-2">
                        ...
                        <button onClick={() => removeItem(idx)} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg">
          ...
          <section className="p-5 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl space-y-4">
            ...
            <button type="button" onClick={handleAddItem} className="w-full py-3 bg-white border border-indigo-100 text-indigo-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 hover:text-white">Add To Invoice</button>
          </section>
        </div>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <div className="flex gap-4 no-print">
            <button 
              onClick={handlePrint}
              className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-800 flex items-center justify-center gap-3 shadow-xl active:scale-95"
            >
              <Printer className="w-5 h-5" /> Execute Print
            </button>
            <button 
              onClick={() => setPreviewPurchase(null)}
              className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 active:scale-95"
            >
              Dismiss
            </button>
          </div>
      ...
      >
        <div className="p-6 md:p-10 bg-slate-100/50 flex flex-col items-center">
          {/* Document Container */}
          <div className="bg-white shadow-xl border border-slate-300 rounded-2xl overflow-hidden p-12">
            <div id="invoice-print-area">
              <div className="w-[148mm] space-y-4 h-[218.5mm] flex flex-col" >
                {/* Professional Header */}
                <div className="flex justify-between items-start border-b border-slate-300 pb-4">
                  <div className="space-y-4">
                    <div>
                      <h1 className="text-2xl font-black text-indigo-600 tracking-tighter leading-none">FabricFlow</h1>
                      <p className="text-[7px] font-black uppercase tracking-[0.3em] text-slate-400 mt-1">Management Systems</p>
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 text-slate-800 font-black text-sm">
                        <Building2 className="w-3.5 h-3.5 text-indigo-500" /> {user?.shopName}
                      </div>
                  ...
                  <div className="text-right">
                    <h2 className="text-4xl font-black text-slate-200/70 uppercase tracking-tighter">Invoice</h2>
                ...
                <div className="grid grid-cols-2">
                  <div className="space-y-1">
                    <p className="text-[8.4px] font-black text-indigo-600 uppercase tracking-widest bg-indigo-50 inline-block px-3 py-1 rounded-full">Procured From</p>
                  ...
                  <div className="space-y-1 text-right">
                    <p className="text-[8.4px] font-black text-slate-400 uppercase tracking-widest">Issue Date</p>
                    ...
                    <div>
                      <p className="text-[8.4px] font-black text-slate-400 uppercase tracking-widest">Payment Status</p>
                    ...
                    <table className="w-full text-left">
                      <thead className="bg-slate-100 border-b-2 border-slate-200">
                        <tr className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">
                          <th className="px-4 py-2.5">Fabric Item</th>
                      ...
                      <tbody className="divide-y divide-slate-200 font-bold text-slate-700 text-xs">
                        {previewPurchase?.items?.map((item: any, i: number) => (
                          <tr key={i} className="hover:bg-slate-50/50">
                            <td className="px-4 py-2.5">
                              <div className="font-medium text-slate-800">{item.articleName}</div>
                            </td>
                            <td className="px-4 py-2.5 text-center text-[10px] uppercase text-slate-400">{item.unit}</td>
                            <td className="px-4 py-2.5 text-center font-black text-slate-900">{item.quantity}</td>
                            <td className="px-4 py-2.5 text-center">${item.price.toLocaleString()}</td>
                            <td className="px-4 py-2.5 text-right font-black text-slate-900 tracking-tight">${(item.quantity * item.price).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {/* Summary Section */}
                <div className="flex flex-col md:flex-row justify-between items-end gap-5 border-t border-slate-300 pt-4 mt-auto">
                  <div className="flex w-full justify-between items-center bg-slate-100 text-slate-700 px-4 py-2 rounded-xl border border-slate-200">
                    <span className="text-[11px] font-medium tracking-wider uppercase">Total</span>
                    <span className="text-md font-bold">{previewPurchase?.amount.toLocaleString()}</span>
                  </div>
                </div>

                {/* footer */}
                <div className="flex justify-between items-end border-t border-slate-300 pt-4 text-slate-600">
                  <span className="text-[11px] tracking-wide">
                    &copy; {new Date().getFullYear()} SparkPair. All rights reserved.
                  </span>
                  <span className="text-[11px] tracking-wide">
                    FabricOS by <strong>SparkPair</strong>
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-6 relative overflow-hidden h-full pb-6">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex gap-3 items-center">
            <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-3 px-8 py-4 bg-indigo-600 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-indigo-700">
              <Plus className="w-5 h-5" /> New Purchase
            </button>
            <button className="flex items-center gap-3 px-8 py-4 bg-slate-200 text-slate-800 font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-slate-300">
              <Filter className="w-5 h-5" /> Filter
            </button>
          </div>
        </header>

        <div className="bg-white rounded-3xl border border-slate-300 h-full shadow-sm overflow-hidden flex flex-col">
          <div className="overflow-x-auto flex-1 custom-scrollbar">
            <table className="w-full text-left">
              <thead className="sticky top-0 z-10 bg-slate-100 shadow-sm">
                <tr className="text-slate-500 font-black text-[10px] uppercase tracking-[0.2em] border-b border-slate-300">
                  <th className="px-8 py-5">Supplier Source</th>
                  <th className="px-8 py-5">Stock Items</th>
                  <th className="px-8 py-5">Invoice Worth</th>
                    ...
                    >
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div className="w-11 h-11 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-black group-hover:bg-indigo-600 group-hover:text-white">
                      ...
                      <td className="px-8 py-5 text-right">
                        <button className="p-3 text-slate-300 group-hover:text-indigo-600">
