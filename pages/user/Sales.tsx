...
import {
  ...
  CheckCircle2, Clock, Trash2, PlusCircle, FileText, Printer, Info, AlertTriangle, ShieldAlert, Layers
} from 'lucide-react';
import toast from 'react-hot-toast';
import { Article, Customer, TransactionItem, StockBatch } from '../../types';
...
const Sales: React.FC = () => {
  ...
  const handleAddItem = () => {
    if (!selectedBatch) return toast.error("Select a physical stock batch.");
    ...
    if (selectedBatch.currentQuantity < qty) {
      return toast.error(`Insufficient stock! Only ${selectedBatch.currentQuantity} ${selectedBatch.unit}s available in this batch.`);
    }
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-6 relative overflow-hidden h-full pb-20 custom-scrollbar">
      <PageHeader 
        title="Batch-Aware Invoicing" 
        subtitle="Precision sales tracking with batch-level margin protection."
        actions={
          <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-3 px-8 py-4 bg-indigo-600 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all active:scale-95">
            <Plus className="w-5 h-5" /> New Batch Sale
          </button>
        }
      />
          ...
          <table className="w-full text-left">
            <thead className="sticky top-0 z-10 bg-white shadow-sm">
              <tr className="text-slate-400 font-black text-[10px] uppercase tracking-[0.2em] border-b border-slate-50">
                ...
                <th className="px-10 py-6">Batch Details</th>
              ...
              {sales.map((s) => (
                <tr key={s.id} onClick={() => setPreviewSale(s)} className="hover:bg-slate-50/80 transition-all group cursor-pointer">
                  <td className="px-10 py-8">
                    ...
                    <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">INV-{s.id.slice(-6)}</div>
                  </td>
                  <td className="px-10 py-8 text-xs font-bold text-slate-500">
                    {s.items?.[0]?.articleName} <span className="text-[9px] text-indigo-400 ml-1">+{s.items?.length - 1 || 0} more</span>
                  </td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Generate Batch Invoice" icon={<PlusCircle className="w-6 h-6" />} maxWidth="max-w-2xl" footer={
        <div className="flex gap-4">
          ...
          <PermissionButton onClick={handleCreateSale} className="flex-1 py-4 shadow-2xl shadow-indigo-100 font-black uppercase tracking-widest text-xs">Authorize Sale</PermissionButton>
        </div>
      }>
        <div className="p-10 space-y-8">
          <Combobox label="Customer" options={customerOptions} value={customerId} onChange={setCustomerId} icon={<User className="w-4 h-4" />} />
              ...
              <div className="divide-y divide-slate-50">
                {invoiceItems.map((item, idx) => (
                  <div key={idx} className="p-4 flex items-center justify-between">
                    <div>
                      <p className="font-bold text-slate-800 text-sm">{item.articleName} <span className="text-[9px] text-slate-400 font-black uppercase ml-2 opacity-50">Batch ID: {item.batchId.slice(-4)}</span></p>
          ...
          <div className="p-8 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2.5rem] space-y-6">
            <Combobox label="Physical Stock Batch" options={batchOptions} value={currentItem.batchId} onChange={id => {
              const b = batches.find(x => x.id === id);
              setCurrentItem({...currentItem, batchId: id, price: (b?.unitCost || 0).toString()});
            }} icon={<Layers className="w-4 h-4" />} />
            
            <div className="grid grid-cols-2 gap-4">
              <input value={currentItem.quantity} onChange={e => setCurrentItem({...currentItem, quantity: e.target.value})} type="number" className="w-full px-5 py-4 bg-white border border-slate-200 rounded-2xl font-bold" placeholder="Qty" />
              <input value={currentItem.price} onChange={e => setCurrentItem({...currentItem, price: e.target.value})} type="number" className={`w-full px-5 py-4 bg-white border rounded-2xl font-bold transition-all ${isSellingAtLoss ? 'border-rose-400 text-rose-600 ring-4 ring-rose-50' : 'border-slate-200'}`} placeholder="Price" />
            </div>

            {isSellingAtLoss && <div className="flex items-center gap-2 text-rose-600 text-[10px] font-black uppercase tracking-widest"><ShieldAlert className="w-4 h-4" /> Warning: Selling Price is below Batch Unit Cost!</div>}
            
            <button type="button" onClick={handleAddItem} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase shadow-lg shadow-indigo-100">Add Batch to Invoice</button>
          </div>
          
          <input value={paidAmount} onChange={e => setPaidAmount(e.target.value)} type="number" className="w-full p-5 bg-white border border-slate-100 rounded-[2rem] font-black text-xl shadow-inner outline-none focus:ring-4 focus:ring-emerald-50 transition-all" placeholder="Cash Received Now..." />
        </div>
      </Modal>

      <Modal isOpen={!!previewSale} onClose={() => setPreviewSale(null)} title="Batch-Level Invoice" icon={<Receipt className="w-6 h-6" />} maxWidth="max-w-4xl">
